...
int ini(void)
{
	...
	SetWindowText(hWnd, TEXT("Flappy bird for pc"));// 使用 API 函数修改窗口名称
	...
	while(1)
	{
		...
		setcolor(WHITE);
		xyprintf(120,480,"Abdiel 2015.11.07); 
...
int game(void)
{
	...
	PIMAGE bird_up_20=newimage();
	PIMAGE bird_middle_20=newimage();
	PIMAGE bird_down_20=newimage();
	PIMAGE bird_up_0=newimage();
	PIMAGE bird_middle_0=newimage();
	PIMAGE bird_down_0=newimage();
	PIMAGE bird_up__20=newimage();
	PIMAGE bird_middle__20=newimage();
	PIMAGE bird_down__20=newimage();
	PIMAGE bird_up__90=newimage();
	PIMAGE bird_middle__90=newimage();
	PIMAGE bird_down__90=newimage();
	...
	getimage(bird_up_0,"依赖包\\图片\\鸟\\上0.png");	
	getimage(bird_middle_0, "依赖包\\图片\\鸟\\中0.png");	
	getimage(bird_down_0, "依赖包\\图片\\鸟\\下0.png");
			
	getimage(bird_up_20, "依赖包\\图片\\鸟\\上20.png");	
	getimage(bird_middle_20, "依赖包\\图片\\鸟\\中20.png");	
	getimage(bird_down_20, "依赖包\\图片\\鸟\\下20.png");
			
	getimage(bird_up__20, "依赖包\\图片\\鸟\\上-20.png");	
	getimage(bird_middle__20, "依赖包\\图片\\鸟\\中-20.png");	
	getimage(bird_down__20, "依赖包\\图片\\鸟\\下-20.png");
			
	getimage(bird_up__90, "依赖包\\图片\\鸟\\上-90.png");	
	getimage(bird_middle__90, "依赖包\\图片\\鸟\\中-90.png");	
	getimage(bird_down__90, "依赖包\\图片\\鸟\\下-90.png");
	...
	while(1)
	{
		...
		while(1)//游戏界面 
		{
			...
			{
				if(a>=0&&a<4)
					putimage_withalpha(NULL,bird_middle_20,100,bird_h);
				if(a>=4&&a<8)
					putimage_withalpha(NULL,bird_up_20,100,bird_h);
				if(a>=8&&a<12)
					putimage_withalpha(NULL,bird_middle_20,100,bird_h);
				if(a>=12&&a<16)
					putimage_withalpha(NULL,bird_down_20,100,bird_h);
			}
			else if(1)
			{
				if(a>=0&&a<4)
					putimage_withalpha(NULL,bird_middle_0,100,bird_h);
				if(a>=4&&a<8)
					putimage_withalpha(NULL,bird_up_0,100,bird_h);
				if(a>=8&&a<12)
					putimage_withalpha(NULL,bird_middle_0,100,bird_h);
				if(a>=12&&a<16)
					putimage_withalpha(NULL,bird_down_0,100,bird_h);
			}
			else if(0)
			{
				if(a>=0&&a<4)
					putimage_withalpha(NULL,bird_middle__20,100,bird_h);
				if(a>=4&&a<8)
					putimage_withalpha(NULL,bird_up__20,100,bird_h);
				if(a>=8&&a<12)
					putimage_withalpha(NULL,bird_middle__20,100,bird_h);
				if(a>=12&&a<16)
					putimage_withalpha(NULL,bird_down__20,100,bird_h);
			}
			else if(0)
			{
				if(a>=0&&a<4)
